  ...
  def save_data(self, data, path):
    with open(path, "w") as writer:
      ...
      for item in data:
        ...
        writer.write(line)
        count += 1
  def predict(self, input_path, output_path, threshold=0.1, is_training_data = False):
    graph = tf.Graph()
...
def process(input_path, output_path):
  ...
  predictor.predict(input_path, output_path, threshold=0.1, is_training_data=False)
