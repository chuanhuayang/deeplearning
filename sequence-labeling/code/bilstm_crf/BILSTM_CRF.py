...
class BILSTM_CRF(object):
    ...
    def __init__(self, num_chars, num_classes, num_steps=200, num_epochs=100, embedding_matrix=None, is_training=True, is_crf=True, weight=False):
        ...
        self.num_steps = num_steps  #sentence长度最长
        self.num_chars = num_chars  #词典大小
        self.num_classes = num_classes  #分类结果数
        
        # placeholder of x, y and weight
        self.inputs = tf.placeholder(tf.int32, [None, self.num_steps]) #[batch_size, 200]
        ...
        self.inputs_emb = tf.reshape(self.inputs_emb, [-1, self.emb_dim])# [batch_size * num_steps, emb_dim]
        self.inputs_emb = tf.split(self.inputs_emb, self.num_steps, 0)# [(num_steps, emb_dim)...]
